...
def test_docker_files():
    ...
    for req_file in requirements_files:
        try:
            ...
            # 주요 패키지 확인
            important_packages = ['fastapi', 'uvicorn', 'mlflow', 'pandas', 'scikit-learn']
            found_packages = []
            ...
            print(f"   중요 패키지: {len(found_packages)}/{len(important_packages)} 발견")
    ...
    try:
        # enhanced.py 임포트 테스트
        # sys.path.append('src')
        from src.utils.enhanced import EnhancedLogger, display_table, HAS_ICECREAM, HAS_RICH
