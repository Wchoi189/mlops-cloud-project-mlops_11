...
def start_monitoring_stack():
    ...
    # 기존 스택 정리
    print("🧹 기존 모니터링 스택 정리...")
    run_command("cd docker && docker-compose -f docker-compose.monitoring.yml down --remove-orphans")
...
def test_api_metrics():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ API 메트릭 수집: {len(found_metrics)}/{len(expected_metrics)} 메트릭 발견")
...
def generate_sample_metrics():
    ...
    try:
        ...
        sample_requests = [
            ...
            ("POST", "http://localhost:8000/predict/movie", {
                "title": "Sample Movie",
                "startYear": 2020,
                "runtimeMinutes": 120,
                "numVotes": 5000
...
def show_useful_commands():
    ...
    commands = [
        ("모니터링 스택 상태 확인", "docker-compose -f docker/docker-compose.monitoring.yml ps"),
        ("모니터링 로그 확인", "docker-compose -f docker/docker-compose.monitoring.yml logs -f"),
        ("Prometheus 로그", "docker-compose -f docker/docker-compose.monitoring.yml logs -f prometheus"),
        ("Grafana 로그", "docker-compose -f docker/docker-compose.monitoring.yml logs -f grafana"),
        ("모니터링 스택 중지", "docker-compose -f docker/docker-compose.monitoring.yml down"),
        ("모니터링 스택 재시작", "docker-compose -f docker/docker-compose.monitoring.yml restart"),
...
def status_check():
    ...
    for name, url in services:
        try:
            response = requests.get(url, timeout=5)
            status = "✅ 정상" if response.status_code in [200, 204] else f"⚠️ HTTP {response.status_code}"
        except:
            status = "❌ 연결 실패"
