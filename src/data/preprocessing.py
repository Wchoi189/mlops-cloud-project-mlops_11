...
class IMDbPreprocessor:
    ...
    def create_genre_features(self, df: pd.DataFrame, genres_list: List[str]) -> pd.DataFrame:
        """장르 원-핫 인코딩 피처 생성"""
    ...
    def create_numerical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 1. 연도 피처 (정규화)
        feature_df['year_normalized'] = pd.to_numeric(feature_df['startYear'], errors='coerce')
        feature_df['year_normalized'] = feature_df['year_normalized'].fillna(feature_df['year_normalized'].median())
        
        # 연도 범위 제한
        feature_df['year_normalized'] = feature_df['year_normalized'].clip(self.min_year, self.max_year)
        
        # 2. 런타임 피처 (결측값 처리)
        feature_df['runtime_minutes'] = pd.to_numeric(feature_df['runtimeMinutes'], errors='coerce')
        runtime_median = feature_df['runtime_minutes'].median()
        feature_df['runtime_minutes'] = feature_df['runtime_minutes'].fillna(runtime_median)
        
        # 3. 투표수 피처 (로그 변환)
        feature_df['votes_log'] = np.log1p(feature_df['numVotes'])  # log(1+x) for stability
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
        ...
        # 숫자형 피처 추가
        numerical_features = ['year_normalized', 'runtime_minutes', 'votes_log', 'title_length']
