...
class ModelEvaluator:
    ...
    def load_model(self, model_path: str, model_type: str = "random_forest"):
        ...
        try:
            ...
            timestamp = model_path_obj.stem.split('_')[-2] + '_' + model_path_obj.stem.split('_')[-1]
    ...
    def evaluate_model(self, X: np.ndarray, y: np.ndarray) -> Tuple[Dict[str, float], np.ndarray]:
        """모델 평가"""
    ...
    def batch_predict(self, movies_df: pd.DataFrame) -> np.ndarray:
        ...
        # 필요한 피처만 추출
        available_features = [col for col in self.feature_names if col in movies_df.columns]
        
        if not available_features:
            raise ValueError(f"필요한 피처가 없습니다. 필요: {self.feature_names}, 사용가능: {list(movies_df.columns)}")
