...
from ..utils.enhanced import (
    ic, EnhancedLogger, ProgressTracker, tools, 
    display_table, enhanced_print, HAS_ICECREAM, HAS_TQDM, HAS_RICH
)
...
class EnhancedMovieRatingTrainer:
    ...
    def load_data(self, data_path: str = "data/processed/movies_with_ratings.csv") -> pd.DataFrame:
        """향상된 진행률 추적으로 데이터 로드"""
        ...
        display_table(
            ...
            [
                ...
                ["평점 범위", f"{df[self.TARGET_COLUMN].min():.1f} - {df[self.TARGET_COLUMN].max():.1f}"]
            ]
        )
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if not available_features:
            self.logger.error(f"필요한 피처를 찾을 수 없습니다! 필요: {self.BASE_FEATURES}")
        ...
        self.logger.success(f"피처 준비 완료: {len(X)}개 샘플, {len(available_features)}개 피처")
    ...
    def train_model(self, X: np.ndarray, y: np.ndarray, 
        ...
        try:
            with mlflow.start_run():
                ...
                # 모델 로깅
                input_example = pd.DataFrame(X_train_scaled[:5], columns=self.feature_names)
    ...
    def save_model(self) -> Dict[str, str]:
        ...
        model_filename = f"enhanced_{type(self.model).__name__.lower()}_{timestamp}.joblib"
...
def enhanced_training_pipeline(data_path: str = "data/processed/movies_with_ratings.csv",
...
def enhanced_model_comparison(data_path: str = "data/processed/movies_with_ratings.csv"):
    """여러 모델 비교를 위한 향상된 함수"""
...
def enhanced_hyperparameter_tuning(data_path: str = "data/processed/movies_with_ratings.csv"):
    """향상된 하이퍼파라미터 튜닝"""
    ...
    for i, params in enumerate(trainer.progress.track(param_combinations, "하이퍼파라미터 테스트")):
        try:
...
def enhanced_data_analysis(data_path: str = "data/processed/movies_with_ratings.csv"):
    ...
    try:
        ...
        if rating_col in df.columns:
            ...
            # 평점 구간별 분포
            bins = [0, 5, 6, 7, 8, 9, 10]
            labels = ['매우 낮음(0-5)', '낮음(5-6)', '보통(6-7)', '좋음(7-8)', '매우 좋음(8-9)', '최고(9-10)']
            
            df['rating_category'] = pd.cut(df[rating_col], bins=bins, labels=labels, include_lowest=True)
            ...
            if len(recent_years) > 0:
                ...
                for start_year, end_year, label in year_groups:
                    ...
                    if len(group_data) > 0:
                        ...
                        year_analysis.append([label, str(movie_count), f"{avg_rating:.2f}"])
        ...
        if genre_columns:
            ...
            for genre_col in genre_columns[:10]:  # 상위 10개 장르만
                ...
                if len(genre_movies) > 0:
                    ...
                    genre_analysis.append([genre_name, str(movie_count), f"{avg_rating:.2f}"])
...
def enhanced_quick_train():
    ...
    try:
        # 기본 설정으로 빠른 훈련
        model_info = enhanced_training_pipeline(
...
def enhanced_full_pipeline():
    ...
    try:
        ...
        display_table(
            ...
            ["단계", "상태", "결과"],
            [
                ["데이터 분석", "✅ 완료", f"{analysis_results['total_movies']:,}개 영화 분석" if analysis_results else "실패"],
                ["모델 비교", "✅ 완료", f"{len(comparison_results)}개 모델 비교" if comparison_results else "실패"],
                ["하이퍼파라미터 튜닝", "✅ 완료", f"최적 RMSE: {tuning_results['best_score']:.4f}" if tuning_results else "실패"]
            ]
        )
    ...
    try:
        ...
        if isinstance(model_data, dict):
            ...
            if 'model' in model_data and hasattr(model_data['model'], 'get_params'):
                ...
                display_table("모델 매개변수", ["매개변수", "값"], param_data[:10])  # 상위 10개만
...
def enhanced_predict_single(title: str, year: int = 2020, runtime: int = 120, votes: int = 5000, 
                        model_path: str = None):
    """단일 영화 예측"""
    ...
    try:
        ...
        if isinstance(model_data, dict):
            ...
            feature_names = model_data.get('feature_names', ['startYear', 'runtimeMinutes', 'numVotes'])
        else:
            model = model_data
        ...
        # 스케일러 적용 (있는 경우)
        scaler_path = str(model_path).replace('enhanced_randomforest', 'enhanced_scaler').replace('enhanced_linear', 'enhanced_scaler')
    ...
    try:
        ...
        display_table(
            ...
            [
                ...
                ["예측 범위", f"{valid_predictions.min():.2f} - {valid_predictions.max():.2f}"],
                ["출력 파일", output_path]
            ]
        )
...
def show_enhanced_help():
    ...
    display_table(
        ...
        ["명령어", "설명", "예제"],
        [
            ["train", "기본 모델 훈련", "python trainer.py train --model_type=random_forest"],
            ["quick_train", "빠른 훈련 (기본 설정)", "python trainer.py quick_train"],
            ["full_pipeline", "전체 파이프라인 실행", "python trainer.py full_pipeline"],
            ["analyze", "데이터 분석", "python trainer.py analyze"],
            ["compare", "모델 성능 비교", "python trainer.py compare"],
            ["tune", "하이퍼파라미터 튜닝", "python trainer.py tune"],
            ["predict", "단일 영화 예측", "python trainer.py predict --title='영화제목' --year=2020"],
            ["batch_predict", "배치 예측", "python trainer.py batch_predict --csv_path=movies.csv"],
            ["model_info", "모델 정보 표시", "python trainer.py model_info --model_path=models/model.joblib"],
...
def enhanced_system_check():
    ...
    # 권장사항
    missing_enhanced = [pkg for pkg in enhanced_packages.keys() 
...
def enhanced_cleanup():
    ...
    try:
        ...
        logger.success(f"{deleted_count}개 파일 삭제 완료 ({total_size / 1024**2:.1f} MB 절약)")
...
def enhanced_export_results(output_dir: str = "results"):
    ...
    """
            ...
            display_table(
                ...
                ["파일", "설명", "크기"],
                [
                    ["model_info.json", "모델 상세 정보", f"{json_path.stat().st_size} bytes"],
                    ["execution_summary.json", "실행 환경 요약", f"{summary_path.stat().st_size} bytes"],
